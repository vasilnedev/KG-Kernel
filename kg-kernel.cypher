...
MERGE (relation:Ontology_Relation {
    ...
    MERGE (rel_has_part:Ontology_Relation {
        ...
        text: "Used to define mereology within the Semantic Layer only. A node reached via a chain of ONTOLOGY_HAS_PART edges from an already-instantiated node inherits that instance status; instantiation does not need to be re-asserted at every part."
    })
    ...
    MERGE (rel_has_instance:Ontology_Relation {
        ...
        text: "Used to instantiate a concrete value of a type within the Semantic Layer only. A part reached via ONTOLOGY_HAS_PART from an already-instantiated node does not need its own ONTOLOGY_HAS_INSTANCE edge; its instance status is implied."
    })
    ...
    MERGE (rel_entity_has_instance:Ontology_Relation {
        GUID: "49923a99-6e75-4a32-8b55-e3b1709b8372",
        label: "ENTITY_HAS_INSTANCE",
        text: "Used to instantiate nodes from the Semantic Layer into the Data Layer, i.e. from a Semantic Layer Ontology_Entity node to a Data Layer node. This is the only relation allowed to cross from the Semantic Layer into the Data Layer."
    })
...
MERGE (pattern:Ontology_Pattern {
    ...
    MERGE (pattern_basic_entity_instantiation:Ontology_Pattern {
        GUID: "126a82e1-7720-4464-aaf1-441033f341b9",
        name: "basicEntityInstantiation",
        text: "Minimal path from an ontology module to a Data Layer instance: an ontology module specializes the kernel's Ontology concept, defines an entity as one of its parts, and that entity is instantiated into the Data Layer. This basic form omits taxonomy and mereology beyond the kernel's own structure, which a real ontology module would typically add. Illustrative example (zoo ontology module): (:Ontology)-[:ONTOLOGY_HAS_TYPE]->(zoo_guide:Ontology)-[:ONTOLOGY_HAS_PART]->(lion:Ontology_Entity)-[:ENTITY_HAS_INSTANCE]->(simba:Lion)"
    })

    MERGE (pattern_entity_relation:Ontology_Pattern {
        GUID: "c3cfc836-c5c2-46e1-b893-f5b1ed51232a",
        name: "entityRelation",
        text: "Shows a module-defined Ontology_Relation connecting Data Layer instances of two of the module's own entities. Ontology_Relation requires being linked to exactly two Ontology_Entity nodes; for now that link is documented in the relation's own text property rather than a formal edge, the same soft-instantiation convention the kernel uses for illustrative facts elsewhere. Illustrative example (zoo ontology module: Animal and Habitat as Ontology_Entity nodes, LIVES_IN as the Ontology_Relation connecting them): (simba:Lion)-[:LIVES_IN]->(savannah:Habitat)"
    })

    MERGE (pattern_taxonomy_mereology:Ontology_Pattern {
        GUID: "722ae47f-2f82-4b7a-9c28-5ebcc9e72c21",
        name: "taxonomyAndMereology",
        text: "Shows the two relations basicEntityInstantiation deliberately omits: taxonomy, where a general entity has a more specific one as a type, and mereology, where a whole entity has another entity as a part. Both keep the kernel's general-to-specific direction: the general or whole node is always the source. Illustrative example (zoo ontology module): (animal:Ontology_Entity)-[:ONTOLOGY_HAS_TYPE]->(lion:Ontology_Entity); (bird:Ontology_Entity)-[:ONTOLOGY_HAS_PART]->(wing:Ontology_Entity)"
    })
...
MERGE (property:Ontology_Property {
    ...
    MERGE (prop_date:Ontology_Property {
...
MERGE (relation)-[:ONTOLOGY_HAS_INSTANCE]->(rel_entity_has_instance)
MERGE (relation)-[:ONTOLOGY_HAS_INSTANCE]->(rel_is_superseded_by)
MERGE (ontology)-[:ONTOLOGY_HAS_PART]->(pattern)
MERGE (pattern)-[:ONTOLOGY_HAS_INSTANCE]->(pattern_basic_entity_instantiation)
MERGE (pattern)-[:ONTOLOGY_HAS_INSTANCE]->(pattern_entity_relation)
MERGE (pattern)-[:ONTOLOGY_HAS_INSTANCE]->(pattern_taxonomy_mereology)
...
MERGE (property)-[:ONTOLOGY_HAS_INSTANCE]->(prop_date)
